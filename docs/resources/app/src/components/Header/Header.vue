<template>
  <v-app-bar
    ...
    <v-toolbar-title :class="$vuetify.theme.dark ? 'primary--text' : null" class="mx-2">
      <v-img max-width="80" src="@/assets/img/akinami_logo.png"></v-img>
    </v-toolbar-title> 
    <!-- <v-toolbar-title :class="$vuetify.theme.dark ? 'primary--text' : null">Master Attendance</v-toolbar-title> -->
    <v-tooltip bottom>
    ...
    <v-menu
      ...
      right>
      <template v-slot:activator="{ on, attrs }">
        <v-btn plain @click="notificationsBadge ? notificationsBadge = !notificationsBadge : ''" v-bind="attrs" v-on="on" class="mr-2">
          Cadastros
        </v-btn>
      </template>
      <v-list>
